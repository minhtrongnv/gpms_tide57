...
fn effScamin(f: s57.Feature, opts: CellOpts) ?i64 {
    ...
    // OpenCPN leaves an object with no SCAMIN effectively ungated (S57Obj::Init
    // defaults Scamin to a very large value). Do the same for SOUNDG: producer
    // SCAMIN controls its admission; never invent a chart-scale/density threshold.
    if (f.objl == OBJL_SOUNDG) return featureScamin(f);
